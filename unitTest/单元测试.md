...
![Go test cover](单元测试.assets/cover.png)
...
通过在`*_test.go`文件中定义`TestMain`函数可以在测试之前进行额外的设置（setup）或在测试之后进行拆卸（teardown）操作。
...
需要注意的是：在调用 `TestMain` 时, `flag.Parse` 并没有被调用。所以如果 `TestMain`  依赖于command-line标志 (包括 testing 包的标记), 则应该显式地调用 `flag.Parse`。
...
4. ![Go Playground](单元测试.assets/example.png)